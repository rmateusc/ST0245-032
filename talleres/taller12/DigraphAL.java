...
public class DigraphAL extends Digraph {
    ...
    public boolean DFS(int source,int destination) {
        boolean [] nodosrecorridos=new boolean[size];
        return DFSAux(source,destination,nodosrecorridos);
    }

    public boolean DFSAux(int source,int dest,boolean [] nodosrecorridos) {
        ...
        if (nodosrecorridos[source]) {
            ...
            if (ya) {
                ...
                boolean r;
                for (int i=0;i<sucesores.size();++i) {
                    r = DFSAux(sucesores.get(i),dest,nodosrecorridos);
                    if (r) {
                        return true;
                    } 
                }
    ...
    public boolean BFS (int source, int destination) {
        ...
        return BFSAux(hijos,destination);
    }
    
    public boolean BFSAux(LinkedList<Integer> hijos, int destination) {
        LinkedList<Integer> nietos=new LinkedList();
        for (Integer sucesor:hijos) {
            ArrayList<Integer> suc=getSuccessors(sucesor);
        ...
        else return BFSAux(nietos,destination);
    }
}